  ...
  return (
    <section className="relative w-full min-h-screen bg-gradient-to-b from-blue-50 via-gray-100 to-gray-300 text-gray-900 flex flex-col items-center overflow-hidden">
      ...
      <div className="relative w-full max-w-5xl mt-16">
        <div className="border-l-4 border-indigo-500 absolute left-1/2 transform -translate-x-1/2 h-full"></div>
        
        <div className="space-y-16">
          {["Founded in 2015", "Expanded to Global Clients", "Achieved 100% Compliance", "AI-Driven Innovations"].map((event, index) => (
            <div
              key={index}
              className={`relative flex items-center w-full ${index % 2 === 0 ? "justify-start" : "justify-end"}`}
              data-aos="fade-up"
            >
              <div className="bg-white p-6 rounded-xl shadow-lg border-2 border-indigo-400 max-w-md transform transition duration-300 hover:scale-105 hover:shadow-2xl">
              ...
              <div className="absolute w-6 h-6 bg-indigo-500 rounded-full left-1/2 transform -translate-x-1/2 border-4 border-white shadow-lg animate-pulse"></div>
            </div>
          ))}
        </div>
      </div>
